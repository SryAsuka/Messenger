...
void IpMsgUdpSession::ipMsgUserExit(IpMsgUser *user)
{
    ...
    command.append(QString::number(g_pkg_seq++).toUtf8() + ":");
    //command.append(QString::number(time(nullptr),10)+":");
    command.append(user->userId + ":");
    command.append(user->userHostName + ":");
    command.append(QString::number(IPMSG_BR_EXIT | IPMSG_ABSENCEOPT, 10).toUtf8() + ":");
...
void IpMsgUdpSession::ipMsgUserRefresh(IpMsgUser *user, unsigned int startip, unsigned int endip)
{
    ...
    command.append(QString::number(IPMSG_BR_ENTRY | IPMSG_ABSENCEOPT, 10).toUtf8() + ":");
...
void IpMsgUdpSession::timerEvent(QTimerEvent *event)
{
    ...
    if (event->timerId() == ipMsgHostDetectTimer && mUser != nullptr && ipMsgHostCurrent != 0
...
void IpMsgUdpSession::ipMsgRespOK(IpMsgUser *user, IpMsgUser *dest)
{
    QByteArray command;
    {
        ...
        command.append(user->userId + ":");
        command.append(user->userHostName + ":");
        command.append(QString::number(IPMSG_RECVMSG | IPMSG_ABSENCEOPT, 10).toUtf8() + ":");
        command.append(QString::number(user->userPktSeq, 10).toUtf8());
        command.append((char) 0);
        command.append((char) 0);
...
void IpMsgUdpSession::ipMsgSendData(IpMsgUser *user, QHostAddress dest, QByteArray data)
{
    QByteArray command;
    {
        ...
        command.append(user->userId + ":");
        command.append(user->userHostName + ":");
        command.append(QString::number(IPMSG_SENDMSG | IPMSG_ABSENCEOPT, 10).toUtf8() + ":");
        command.append(data);
        command.append((char) 0);
        command.append((char) 0);
        // ipMsgPktSend(dest, IPMSG_PORT, command);
...
void IpMsgUdpSession::ipMsgSendFiles(IpMsgUser *user, QHostAddress dest, QList<fileEntryT *> files)
{
    ...
    command.append(user->userHostName + ":");
    command.append(
        QString::number(IPMSG_SENDMSG | IPMSG_ABSENCEOPT | IPMSG_FILEATTACHOPT, 10).toUtf8() + ":");
    command.append((char) 0);
    for (int i = 0; i < files.count();) {
        command.append(QString::number(files.at(i)->fileId, 10).toUtf8() + ":");
        command.append((files.at(i)->info.fileName).toUtf8() + ":");
        command.append("0" + QString::number(files.at(i)->info.size, 16).toUtf8() + ":");
        command.append(QString::number(files.at(i)->info.metadataChangeTime, 16).toUtf8() + ":");
        command.append("1:");
...
void IpMsgUdpSession::ipMsgRespBr(IpMsgUser *user, IpMsgUser *dest)
{
    ...
    command.clear();
    {
        ...
        command.append(user->userId + ":");
        command.append(user->userHostName + ":");
        command.append(QString::number(IPMSG_ANSENTRY, 10).toUtf8() + ":");
