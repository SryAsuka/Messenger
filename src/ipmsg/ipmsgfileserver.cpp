...
IpMsgFileServer::IpMsgFileServer(QObject* parent)
    : QObject(parent)
{
...
void IpMsgFileServer::fileServerNewConnection()
{
    ...
    if (nullptr == sock)
        return;
    qDebug() << "Client" << sock->peerAddress().toString() << "Port" << sock->peerPort()
             << "connected";
...
void IpMsgFileServer::fileServerReadyRead()
{
    ...
    qDebug() << "Client" << readSocket->peerAddress() << "Port" << readSocket->peerPort()
             << "send data";
    buffer = readSocket->readAll();
    qDebug() << buffer;
    //readSocket->close();
    emit ipMsgFileServerDataReady(readSocket->peerAddress().toIPv4Address(),
                                  readSocket->peerPort(),
                                  buffer);
}
...
void IpMsgFileServer::fileServerSockDisconnect()
{
    ...
    qDebug() << "Client" << srcSocket->peerAddress().toString() << "Port" << srcSocket->peerPort()
             << "disconnected";
    qDebug()
        << "Tell send thread, stop send data to this client and delete sock and connect from queue";
    return;
}
...
QTcpSocket* IpMsgFileServer::fileServerSockMatch(quint32 host, quint16 port)
{
    for (int i = 0; i < mSocks.count(); i++) {
        if (mSocks.at(i)->peerAddress().toIPv4Address() == host && mSocks.at(i)->peerPort() == port)
            return mSocks.at(i);
    }
    return nullptr;
}
