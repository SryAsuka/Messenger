    ...
    , ui(new Ui::FormChat)
{
    ...
    connect(ui->tableWidgetSendFileList,
            SIGNAL(customContextMenuRequested(const QPoint)),
            this,
            SLOT(on_tableWidgetSendFileList_customContextMenuRequested(const QPoint)));
    connect(ui->tableWidgetRecvFileList,
            SIGNAL(customContextMenuRequested(const QPoint)),
            this,
            SLOT(on_tableWidgetRecvFileList_customContextMenuRequested(const QPoint)));
...
void FormChat::addShareFile()
{
    ...
    for (int i = 0; i < files.count(); i++) {
        ...
        row1Item->setForeground(QBrush(QColor("Blue")));
        ...
        fileNode->info.metadataChangeTime = fileinfo.metadataChangeTime().toSecsSinceEpoch();
#else
        ...
        qDebug() << ui->tableWidgetSendFileList->rowCount() << fileNode->info.fileName + "|"
                 << fileNode->info.size;
    }
}
